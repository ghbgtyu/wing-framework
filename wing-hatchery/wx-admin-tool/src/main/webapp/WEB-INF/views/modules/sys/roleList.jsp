...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<tr><th>角色名称</th><th>备注</th><th>是否全平台权限</th><shiro:hasPermission name="sys:role:edit"><th>操作</th></shiro:hasPermission></tr>
		<c:forEach items="${list}" var="role">
			<tr>
				<td><a href="form?id=${role.id}">${role.name}</a></td>
                <td>${role.remarks}</td>
                <td>
                    <c:if test="${role.isGlobal == 1}">是</c:if>
                    <c:if test="${role.isGlobal == 0}">否</c:if>
                </td>
