...
<form:form id="searchForm" modelAttribute="recharge" action="${ctx}/tools/recharge/" method="post"
    ...
    <label>申请时间：</label>
    <input type="text" name="createTimeStart" class="input-small" readonly="readonly" maxlength="20"
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed">
        ...
        <c:forEach items="${page.list}" var="item">
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${item.rechargeStatus == 2}"><span
                                class="label label-warning">${fns:getDictLabel(item.rechargeStatus, 'recharge_status', '无')}</span></c:when>
                        <c:when test="${item.rechargeStatus == 1}"><span
                                class="label label-success">${fns:getDictLabel(item.rechargeStatus, 'recharge_status', '无')}</span></c:when>
                        <c:when test="${item.rechargeStatus == 0}"><span
                                class="label label-info">${fns:getDictLabel(item.rechargeStatus, 'recharge_status', '无')}</span></c:when>
