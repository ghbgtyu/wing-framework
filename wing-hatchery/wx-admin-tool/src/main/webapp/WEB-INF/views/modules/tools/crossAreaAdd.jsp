            ...
            <select id="crossAreaType" name="crossAreaType">
                <c:forEach items="${fns:getDictList('crossType')}" var="item">
                    <option value="${item.value}"
                            <c:if test="${item.value eq crossAreaType}">selected="selected"</c:if>>${item.label}</option>
                </c:forEach>
            </select>
        </div>
    </div>
        ...
        <div class="controls">
            <select id="crossServerId" name="crossServerId">
                <c:if test="${crossArea!=null}">
                    <option value="${crossArea.crossServer.worldId}"
                            selected="selected">${crossArea.crossServer.worldName}</option>
                </c:if>
                <c:forEach items="${crossServerToPage}" var="item">
                    <option value="${item.worldId}"
                            <c:if test="${item.worldId==wroldId}">selected="selected"</c:if>>${item.worldName}</option>
                </c:forEach>
            </select>
        </div>
    </div>
        ...
        <div class="controls">
            <select id="gameServerId" name="gameServerId" multiple="multiple">
                <c:forEach items="${gameServerToPage}" var="item">
                    <option value="${item.worldId}"
                            <c:if test="${fn:indexOf(gameServersId,item.worldId)>0}">selected="selected"</c:if>>${item.worldName}</option>
                </c:forEach>
