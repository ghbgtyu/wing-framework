...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="game.notice.edit">
			<li class="active"><a href="${ctx}/tools/gameNotice/form?id=${gameNotice.id}">${not empty gameNotice.id?'修改':'添加'}公告</a></li>
		</shiro:hasPermission>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="gameNotice" action="${ctx}/tools/gameNotice/save" method="post" class="form-horizontal">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                ...
                -
                <input name="finishTime" maxlength="50"  class="input-small required" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'#F{$dp.$D(\'startTime\')}'})"/>
            </div>
        </div>
        <div class="control-group">
        ...
        <div class="control-group">
            ...
            <div class="controls" id="gameServerTreeDiv">
                <input id="serverIds" name="serverIds" readonly/><input type="button" class="btn btn-primary" value="选择服务器" onclick="openSeverDialog()"></button>
            </div>
        </div>
