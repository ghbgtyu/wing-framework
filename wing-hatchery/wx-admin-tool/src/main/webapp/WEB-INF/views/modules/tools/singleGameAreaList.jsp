...
<body>
    ...
    <table id="contentTable" class="table table-bordered table-condensed">
        <thead><tr><th hidden="hidden"><input type="checkbox" id="selectall" name="selectall"/></th><th>服务器编号</th><th>服务器ip</th><th>服务器名称</th><th>状态</th><th>开服时间</th><th>平台</th></tr></thead>
        <tbody>
        <c:forEach items="${page.list}" var="gameServer" varStatus="status">
            <tr>
                ...
                <td>				  
				   <c:choose>
                    <c:when test="${gameServer.status == 0}"><span class="label label-info">${fns:getDictLabel(gameServer.status, "server_status","未知")}</span></c:when>
                    <c:when test="${gameServer.status == 1}"><span class="label label-warning">${fns:getDictLabel(gameServer.status, "server_status","未知")}</span></c:when>
                    <c:otherwise><span class="label label-important">${fns:getDictLabel(gameServer.status, "server_status","未知")}</span></c:otherwise>
                   </c:choose></td>
