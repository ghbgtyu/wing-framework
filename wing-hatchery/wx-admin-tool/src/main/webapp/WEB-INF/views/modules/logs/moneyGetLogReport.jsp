...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/log/moneyGetLog/moneyGetLogReport">货币获取日志</a></li>
</ul>
<form id="searchForm" action="${ctx}/log/moneyGetLog/moneyGetLogReport" method="post" class="breadcrumb form-search">
    ...
    <select name="operateType">
        ...
        <c:forEach items="${fns:getOperaTypeMap()}" var="item">
            <option value="${item.key}"
                    <c:if test="${item.key eq paramMap.operateType}">selected="selected"</c:if>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <tbody>
    <c:forEach items="${moneyGetLog.list}" var="item">
        <tr>
...
<div class="pagination">${moneyGetLog}</div>
