...
<div id="serverDialog" class="">
    <div class="breadcrumb form-search">
        ...
        <div class="pop_layer_cont">
            <input type="checkbox" class="btn_select_16" onclick="allSelectOrNot(this)"/>选中所有服
    ...
    <c:forEach items="${gamePlatform}" var="platform" varStatus="status">
        ...
        <div class="modal-form modal-example select2-container-multi" style="background: #fff4e5">
            <div class="modal-header">
                <input type="checkbox" onclick="" id="selectAll_${status.index}" class="game-server-sort-name"/>
                <small class="label label-info">${platform.name}</small>
            </div>
            <div id="game-server-list-${status.index}" class="modal-body">
                <c:forEach items="${platform.gameServerList}" var="item" varStatus="serverIdStatus">
                    <input type="checkbox" class="ids_${status.index}" name="ids_${status.index}"
                           value="${item.worldId}" id="${item.worldId}"/>${item.worldName}【${item.worldId}】
                </c:forEach>
            </div>
        </div>
