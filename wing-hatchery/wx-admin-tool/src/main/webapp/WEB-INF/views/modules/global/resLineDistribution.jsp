...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/global/tradeController/resLineDistribution">资源线消费分布</a></li>
</ul>
...
<div class="row-fluid">
    <div class="span12 panel panel-primary">
        <div class="panel-heading">资源线消费分布（元宝）</div>
            ...
            <table id="resLine" class="table table-striped table-bordered table-condensed">
                ...
                <c:forEach items="${reslist}" var="item">
                    <tr>
                        ...
                        <td>资源线${item.res_line }</td>
                        <td>${item.yb }</td>
                        <td><c:if test="${totalYb >0 }"><fmt:formatNumber value="${item.yb*100/totalYb }"
                                                                          pattern="#0.00"/>%</c:if></td>
                        <td>${item.num }</td>
                        <td><c:if test="${ybNum > 0 }"><fmt:formatNumber value="${item.num*100/ybNum }"
                                                                         pattern="#0.00"/>%</c:if></td>
                    </tr>
                </c:forEach>
                <tr>
...
<div class="row-fluid">
    <div class="span12 panel panel-primary">
        <div class="panel-heading">资源线消费分布（绑定元宝）</div>
        <div class="" style="width:100%;height:300px;line-height:300px;overflow:auto;overflow-x:hidden;">
            <table id="band-yb" class="table table-striped table-bordered table-condensed">
                ...
                <c:forEach items="${reslist}" var="item">
                    ...
                    <tr>
                        ...
                        <td>资源线${item.res_line }</td>
                        <td>${item.bind_yb }</td>
                        <td><c:if test="${totalBind > 0 }"><fmt:formatNumber value="${item.bind_yb*100/totalBind }"
                                                                             pattern="#0.00"/> %</c:if></td>
                        <td>${item.bind_num }</td>
                        <td><c:if test="${bindNum > 0 }"><fmt:formatNumber value="${item.bind_num*100/bindNum }"
                                                                           pattern="#0.00"/> %</c:if></td>
                    </tr>
                </c:forEach>
                <tr>
