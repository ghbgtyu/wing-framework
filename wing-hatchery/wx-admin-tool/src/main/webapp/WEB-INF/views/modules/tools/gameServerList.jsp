...
<body>
    ...
    <form:form id="searchForm" modelAttribute="gameServer" action="${ctx}/tools/gameServer/" method="post" class="breadcrumb form-search">
        ...
        <label><spring:message code="game.server.id"/>：</label><form:input path="serverId" htmlEscape="false" maxlength="50" class="input-small"/>
        ...
        <c:forEach items="${page.list}" var="gameServer" varStatus="status">
            <tr>
                ...
                <td>
                    ...
                    <a href="${ctx}/tools/gameServer/delete?id=${gameServer.id}" onclick="return confirmx('确认要删除该服务器吗？', this.href)">删除</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
