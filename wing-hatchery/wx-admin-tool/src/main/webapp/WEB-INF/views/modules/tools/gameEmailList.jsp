...
<form:form id="searchForm" modelAttribute="gameEmail" action="${ctx}/tools/gameEmail/" method="post"
    ...
    <label>申请时间：</label>
    <input type="text" name="createTimeStart" class="input_search" size="10" readonly="readonly" maxlength="20"
           value="${paramMap.createTimeStart}" id="startDatePicker"
        ...
        <c:forEach items="${page.list}" var="gameEmail">
            <tr>
                ...
                <td style="overflow:hidden;white-space:nowrap;"><a
                        title="${fn:replace(gameEmail.attachments,"\"","")}">${gameEmail.attachments}</a></td>
                <td>${gameEmail.serverIds}</td>
                <td style="overflow:hidden;white-space:nowrap;"><a
                        title="${gameEmail.receiverNames}">${gameEmail.receiverNames}</a></td>
                <td style="overflow:hidden;white-space:nowrap;"><a
                        title="${gameEmail.receiverUserIds}">${gameEmail.receiverUserIds}</a></td>
                <td>${gameEmail.createName}</td>
                <td>${gameEmail.approveName}</td>
                <td><fmt:formatDate value="${gameEmail.createTime}" type="both"/></td>
                <td>
                    <c:choose>
                        <c:when test="${gameEmail.emailStatus == 2}"><span
                                class="label label-warning">${fns:getDictLabel(gameEmail.emailStatus, 'email_status', '无')}</span></c:when>
                        <c:when test="${gameEmail.emailStatus == 1}"><span
                                class="label label-success">${fns:getDictLabel(gameEmail.emailStatus, 'email_status', '无')}</span></c:when>
                        <c:when test="${gameEmail.emailStatus == 0}"><span
                                class="label label-info">${fns:getDictLabel(gameEmail.emailStatus, 'email_status', '无')}</span></c:when>
                ...
                <td>
                    <c:if test="${gameEmail.emailStatus == 0}">
                        <shiro:hasPermission name="game.email.send">
                            <a href="${ctx}/tools/gameEmail/send?id=${gameEmail.id}"
                               onclick="return confirmx('确认要发送邮件吗？', this.href)">发送</a>
                        </shiro:hasPermission>
                        <shiro:hasPermission name="game.email.cancel">
                            <a href="${ctx}/tools/gameEmail/cancel?id=${gameEmail.id}"
                               onclick="return confirmx('确认要取消该邮件吗？', this.href)">取消</a>
                        </shiro:hasPermission>
                    </c:if>
                    <c:if test="${gameEmail.emailStatus == 2}">
                        <a href="${ctx}/tools/gameEmail/recover?id=${gameEmail.id}"
                           onclick="return confirmx('确认要恢复该邮件吗？', this.href)">恢复</a>
                    </c:if>
                </td>
            </tr>
        </c:forEach>
