...
<body>
	...
	<form id="searchForm" action="${ctx}/tools/crossArea/crossAreaManage" method="post" class="breadcrumb form-search">
		...
		&nbsp;<input id="btnExport" class="btn btn-primary" type="button" value="导出" onclick="exportXls('${ctx}/tools/crossArea/exportXls')"/>
		&nbsp;<input id="btnSubmit" class="btn btn-primary" type="button" value="删除跨服的服务器列表" onclick="deleteCrossAreaList('${ctx}/tools/crossArea/deleteAllCrossArea')"/>
		
	</form>
	<tags:message content="${message}"/>
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
		<tr><th>类型</th><th>跨服ID</th>	<th>跨服名字</th>	<th>赛区名称</th> <th>IP</th> <th>port</th>
			<th>
				<c:choose>
		...
		<c:forEach items="${crossAreaList}" var="item">
			<tr>
				...
				<td><a href="${ctx}/tools/crossArea/deleteCrossAreaById?crossType=${item.crossType}&crossAreaId=${item.crossServer.worldId}">删除</a>
					<a href="${ctx}/tools/crossArea/updateCrossArea?crossType=${item.crossType}&crossAreaId=${item.crossServer.worldId}">修改</a> </td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
