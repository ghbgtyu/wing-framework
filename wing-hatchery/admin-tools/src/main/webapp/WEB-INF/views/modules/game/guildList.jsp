...
<body>
	...
	<form id="searchForm" action="${ctx}/game/guildLog/getGuildList" method="post" class="breadcrumb form-search">
		...
		<input id="btnExport" class="btn btn-primary" type="button" value="导出" onclick="exportXls('${ctx}/game/guildLog/exportXls')"/>
		</shiro:hasPermission>
	</form>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="item">
			<tr>
				...
				<a href="${ctx}/game/guildLog/deleteGuild?id=${item.id}" onclick="return confirmx('确认要删除该公会吗？', this.href)">删除</a>
				</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
