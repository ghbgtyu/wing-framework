...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="game.notice.edit"><li><a href="${ctx}/tools/gameNotice/form">添加公告</a></li></shiro:hasPermission>
	</ul>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<tr><th>公告类型</th><th width="20%">公告内容</th><th>服务器</th><th>总循环次数</th><th>间隔</th><th>开始时间</th><th>结束时间</th><th>状态</th><th>创建人</th><th>创建时间</th><th>操作</th></tr>
		<c:forEach items="${page.list}" var="gameNotice">
			<tr>
				<td>${fns:getDictLabel(gameNotice.noticeType, 'notice_type', '无')}</td></td>
                ...
                <td>
                    <c:choose>
                        <c:when test="${gameNotice.noticeStatus == 1}"><span class="label label-success">${fns:getDictLabel(gameNotice.noticeStatus, 'notice_status', '无')}</span></c:when>
                        <c:when test="${gameNotice.noticeStatus == 0}"><span class="label label-info">${fns:getDictLabel(gameNotice.noticeStatus, 'notice_status', '无')}</span></c:when>
				...
				<td>
					...
					<c:if test="${gameNotice.noticeStatus == 0}"><a href="${ctx}/tools/gameNotice/publish?id=${gameNotice.id}" onclick="return confirmx('确认要发布该公告吗？', this.href)">发布</a></c:if>
					</shiro:hasPermission>
                    ...
                    <c:if test="${gameNotice.noticeStatus == 1 || gameNotice.noticeStatus ==2}">
                        <a href="${ctx}/tools/gameNotice/delete?id=${gameNotice.id}" onclick="return confirmx('确认要取消该公告吗？', this.href)">取消</a>
                    </c:if>
					</shiro:hasPermission>
				</td>
			</tr>
