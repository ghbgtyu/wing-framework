...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
		<tr><th>平台</th> <th>服务器</th> <th>玩家ID</th> <th>角色名</th> <th>总充值金额</th>
			<th>注册时间</th> <th>首次充值时间</th> <th>最后充值时间</th> <th>游戏天数</th> <th>总充值次数</th>
			<th>元宝存量</th> <th>未登录天数</th> <th>登录状态</th> <th>未支付天数</th> <th>支付状态</th></tr>
		</thead>
		<tbody>
		<c:forEach items="${daZhongRMonitor.list}" var="item">
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${item.sub_last_login_time>warnLogin}"><span class="label label-important">预警</span></c:when>
				...
				<td>
					<c:choose>
						<c:when test="${item.sub_last_pay_time>warnCharge}"><span class="label label-important">预警</span></c:when>
						<c:otherwise><span class="label label-info">正常</span></c:otherwise>
					</c:choose>
				</td>
			</tr>
