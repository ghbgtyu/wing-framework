...
public class Redis implements IRediser {
	...
	public Redis(String host, int port, int timeout, String password, int index) {
		// 通过config配置连接池参数
		this(GenericObjectPoolConfig.DEFAULT_MAX_TOTAL, host, port, DEFAULT_TIMEOUT, null, index);
	}
	...
	public Redis(int connection, String host, int port, int timeout, String password, int database) {
		...
		LogUtil.info("Redis 连接信息:host={},port={},database={},timeout={}ms,password={}", new Object[] { host, port, database, timeout, password });
		ping();
	}
	...
	public void ping() {
		try (Jedis j = pool.getResource()) {
			String ret = j.ping();
			if (ret.equals("PONG"))
				LogUtil.info("Redis服务器连接正常");
			else
				LogUtil.info("Redis服务器连接异常");
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public static void migrate(String fromHost, int fromPort, int fromIndex, String toHost, int toPort, int toIndex, int timeout, String... list) {
		...
		LogUtil.info("数据迁移开始 fromHost={},fromPort={},fromIndex={},toHost={},toPort={},toIndex={},timeout={},list={}", fromHost, fromPort, fromIndex, toHost,
		...
		redis.destory();
		LogUtil.info("数据迁移完成 inerval={},ret={}", (System.nanoTime() - start) / 1000000f, ret);
	}
	...
	public static void main(String[] args) throws InterruptedException {
		Redis redis = new Redis("192.168.1.21", 6399, 0);
		...
		try (PrintStream ps = new PrintStream(new FileOutputStream("e:/keys.txt"));) {
